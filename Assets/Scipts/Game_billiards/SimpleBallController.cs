...
public class SimpleBallController : MonoBehaviour
{
    ...
    [SerializeField] static bool isAnyBallPlaying = false;           //어떤 공이라도 턴 중인지
    [SerializeField] static bool isAnyBallMoveing = false;           //어떤 공이라도 움직이는지
    ...
    {
        return rb.velocity.magnitude > 0.2f;            //공이 속도를 
    }
    ...
    void UpdateArrow()              //화살표 업데이트
    {
        ...
        SpriteRenderer sr = arrow.GetComponent<SpriteRenderer>();       //화살표 색상 변경
        ...
        if (distance > 10f)         //최소 거리 이상 드래그 했을 때
        {
            ...
            float angle = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg;            //방향을 잡아 주는 공식 (벡터의 방향을 잡아주는 코드)
